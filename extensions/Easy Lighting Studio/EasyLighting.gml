...
for (var i=0;i<size;++i)
{
    ...
    if (ID>=0)
    {
        if (surface_exists(ds_list_find_value(__ls_surface_finals,i)))
            surface_free(ds_list_find_value(__ls_surface_finals,i));
        ds_list_replace(__ls_surface_finals,i,surface_create(view_wview[ID],view_hview[ID]));
    }
    else
    {
        if (surface_exists(ds_list_find_value(__ls_surface_finals,i)))
...
for (var i=0;i<surfaces;++i)
{
    ...
    if (ID>=0)
    {
        if (surface_get_width(surf)!=view_wview[ID]||surface_get_height(surf)!=view_hview[ID])
            ls_view_recalculate();
    }
    else
    {
